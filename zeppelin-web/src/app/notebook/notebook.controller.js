...
function NotebookCtrl($scope, $route, $routeParams, $location, $rootScope,
  ...
  $scope.restartSparkInterpreter = function() {
    const thisConfirm = BootstrapDialog.confirm({
      closable: false,
      closeByBackdrop: false,
      closeByKeyboard: false,
      title: '',
      message: 'Do you want to restart ' + _.escape('spark') + ' interpreter?',
      callback: function(result) {
        if (result) {
          let payload = {
            'noteId': $scope.note.id,
          ...
          $http.put(baseUrlSrv.getRestApiBase() + '/interpreter/setting/restart/' + 'spark', payload)
            .success(function(data, status, headers, config) {
              let index = _.findIndex($scope.interpreterSettings, {'id': 'spark'});
              $scope.interpreterSettings[index] = data.body;
              thisConfirm.close();
            }).error(function(data, status, headers, config) {
              thisConfirm.close();
